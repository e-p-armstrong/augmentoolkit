...
# Helper function to build the sources string
def _build_sources_string(source_items_data):
    # source_items_data is a list of tuples:
    # [(original_meta, [rag_meta1, rag_meta2]), ...]
    ...
    for original_meta, rag_metas_list in source_items_data:
        ...
        for rag_meta in rag_metas_list:
            if str(rag_meta) not in currently_added:
                sources_string_parts.append(f"- {str(rag_meta)}")
                currently_added.add(str(rag_meta))
    return "\n".join(sources_string_parts)
...
def save_plain_qatuples(
    ...
    for text_entry in multi_item_dicts:
        ...
        conversations_rag = []
        dict_list = text_entry["sorted"]
        paragraph_text = text_entry["sorted"][0]["text"]
        ...
        for d in dict_list:
            # print("Q")
            # print(d["question"])
            # print("IS QUESTION YES, PLEASE?")
            # print(d["question"] == "Yes, please.")
            is_question_yes_please = d["question"] == "Yes, please."
            if is_question_yes_please:
                # print("YES, PLEASE!")
                new_q = random.choice(
                    [
                        "Yes, please.",
            ...
            conversations.append({"from": "gpt", "value": a + sources_str})
            conversations_rag.append({"from": "human", "value": q})
            conversations_rag.append({"from": "gpt", "value": a + sources_str})
    ...
    for idx in range(0, len(single_item_dicts), items_per_conversation):
        ...
        for entry in combined_entries:
            ...
            # print("IS QUESTION YES, PLEASE?")
            # print(qa_pair["question"] == "Yes, please.")
            q = qa_pair["question"]
            a = qa_pair["answer"]
