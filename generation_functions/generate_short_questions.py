...
def generate_short_questions(text, existing_question_tuples,logic_llm):
    ...
    if existing_question_tuples:
        existing_qs = "\n".join(["Existing question: " + t[0] + "\nAnswer: " + t[1] for t in existing_question_tuples]) # stop duplicates from being generated
    else:
        existing_qs = None
...
"""
        # print("DEBUG\n\n" + decision_prompt)
        completion = logic_llm(question_prompt, max_tokens=2000, stop=["</s>"], echo=True, grammar=questions_grammar,temperature=0.2)["choices"][0]["text"]
