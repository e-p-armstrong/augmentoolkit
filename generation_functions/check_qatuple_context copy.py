...
"""
        # print("DEBUG\n\n" + decision_prompt)
        try:
            completion = logic_llm(decision_prompt, max_tokens=8000, stop=["</s>"], echo=True, grammar=check_qatuple_context_grammar, temperature=0.2)["choices"][0]["text"]
            ...
            if "Reword" in determination or "reword" in determination:
                return ("New QA Tuple","New","New","New") # TODO search for the reworded question and answer
...
if __name__ == "__main__": # test
    logic_llm = Llama(model_path=LOGICAL_MODEL,n_ctx=8000,rope_freq_scale=0.33,n_gpu_layers=100) # load the logical LLM and offload everything
