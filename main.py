...
multi_turn_convs_info_dir = obj_conf['PATH']['OUTPUT'] + "/multi_turn_convs_info"  # we generate all the information fed to the multiturn prompt, and generate the actual multiturn prompt, separately; since every step but the last is capable of being done by a 13b
...
engine_wrapper = EngineWrapper(model=obj_conf['API']['LOGICAL_MODEL'],api_key=obj_conf['API']['API_KEY'],base_url=obj_conf['API']['BASE_URL'])
...
async def filter_paragraphs(paragraphs_processed, judged_worthy_for_questions, engine_wrapper, output_dir, take_subset= True, use_filenames= True, rtwl=run_task_with_limit):
    await control_flow_functions.filter_all_questions(paragraphs_processed, judged_worthy_for_questions, engine_wrapper, output_dir, take_subset=take_subset, use_filenames=use_filenames, rtwl=rtwl)

# Call the async function
asyncio.run(filter_paragraphs(paragraphs_processed, judged_worthy_for_questions, engine_wrapper, output_dir, take_subset=obj_conf['SYSTEM']['USE_SUBSET'], use_filenames=obj_conf['SYSTEM']['USE_FILE_NAMES'], rtwl=run_task_with_limit))
