...
You'll find an example of a question generated, from plaintext, entirely by a 70b, in conv_0.json. This was generated using HG Wells' "A Short History of the World" which is absurdly hard for a model to make questions from for a number of reasons, and which is why this is a good example. Note the autogenerated character card, scenario, question and answer.

If it doesn't work for you it means I forgot to include something in this repo. Shout at me and I'll add it!
*I added a try-except around the single-turn example gen loop and I haven't tested that yet. I doubt I'd fuck up something that simple but it's 6AM so who knows. I'll fix it in the morning if that's the case.*
Imma sleep now.
